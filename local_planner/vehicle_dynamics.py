...
class VehicleDynamics:
    ...
    def get_current_state(self):
        return np.array([self.x_pos, self.y_pos, self.velocity, self.yaw])

    def update_state(self, control_vec):
        """ """
        acceleration, steering_angle_delta = control_vec

        self.x_pos += (self.velocity * np.cos(self.yaw)) * self.dt
        self.y_pos += (self.velocity * np.sin(self.yaw)) * self.dt
        self.velocity += acceleration * self.dt

        self.yaw += self.velocity / self.WHEEL_BASE_LENGTH * np.tan(steering_angle_delta) * self.dt
    ...
    def prediction_motion(self, ctrl_acc, ctrl_delta, reference_x, mpc_horizon):
        ...
        current_yaw, current_vel = self.yaw, self.velocity
        ...
        for (ctrl_acc_val, ctrl_delta_val, idx) in zip(ctrl_acc, ctrl_delta, range(1, mpc_horizon + 1)):
            ...
            x_prediction[2, idx] = self.velocity
            x_prediction[3, idx] = self.yaw

        self.x_pos, self.y_pos = current_x_pos, current_y_pos
        self.yaw, self.velocity = current_yaw, current_vel
